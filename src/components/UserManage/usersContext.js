  ...
  return (
    <Grid2 container justifyContent="center" alignItems="center" style={{ minHeight: '50vh', minWidth: '650px' }}>
      <Grid2 item xs={12} sm={10} md={8} lg={6}>
        <Paper elevation={3} sx={{ padding: 4, width: '450px', margin: '0 auto' }} style={{ background: '#CCCCCC' }}>
...
export const UserEditForm = ({ accessToken, refreshToken }) => {
  ...
  const { id } = useParams()
  const { userActiveData, setUserActiveData, setUserInactiveData, setRoles, roles, fetchUsers } = useData();
  const userToEdit = userActiveData.find((user) => user._id === parseInt(id, 10));
...
console.log("userToEditRol", userToEdit?.rol_w_id[0].id? userToEdit.rol_w_id[0].id : 'null');
    setFormDataChange({
      user: userToEdit?.user || "",
      email: userToEdit?.email || "",
      password: "",
  ...
  useEffect(() => {
    ...
    if (formDataChange.user !== formData.user || formDataChange.email !== formData.email || formDataChange.password !== formData.password || formDataChange.rol_id !== formData.rol_id) {
  ...
  const editUser = async ({ formDataChange }) => {
    ...
    try {
      const data = await API_EditUser({ accessToken, refreshToken, formDataChange });
      console.log(data);
      if (data["response"] === "user_updated") {
        ...
        // Aquí puedes volver a cargar los usuarios desde la API
        fetchUsers(setUserActiveData, setUserInactiveData, accessToken, refreshToken, setRoles);
        toastShow({ message: `Usuario ${formDataChange.user} actualizado correctamente`, type: "success",  id: `user-${id}`});
        navigate('/mnemosine/administration/user_manage/user/users_active');
      } else {
  ...
  return (
    <Grid2 container justifyContent="center" alignItems="center" style={{ minHeight: '50vh', minWidth: '650px', marginTop: '20px' }}>
      <Grid2 item xs={12} sm={10} md={8} lg={6} >
        <Paper elevation={3} sx={{ padding: 4, width: '450px', margin: '0 auto' }} style={{ background: '#CCCCCC' }}>
          <Typography variant="h5" fontWeight="bold" gutterBottom >
            Editar Usuario
